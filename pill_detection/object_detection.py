...
class ObjectDetection(object):
    ...
    def postprocess(self, prediction_outputs):
        ...
        return [{'probability': round(selected_probs[i], 8),
         'tagId': int(selected_classes[i]),
         'tagName': self.labels[selected_classes[i]],
         'boundingBox': {
             'left': round(selected_boxes[i][0], 8),
             'top': round(selected_boxes[i][1], 8),
             'width': round(selected_boxes[i][2], 8),
             'height': round(selected_boxes[i][3], 8)
         }
